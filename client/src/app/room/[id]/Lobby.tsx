...
export default function Lobby() {
  ...
  return (
    <div className="mx-auto w-full max-w-3xl animate-fade-in rounded-2xl border border-slate-600/60 bg-slate-800 p-6 text-slate-100 shadow-xl">
      <header className="mb-6 flex flex-col items-center gap-2 text-center">
        ...
        <div className="flex items-center gap-3 text-xs font-medium text-slate-400">
          <span className="inline-flex items-center gap-1 rounded-full bg-slate-600/30 px-2 py-0.5 ring-1 ring-inset ring-slate-500/40">Players: <span className="text-slate-200 font-semibold">{playerCount}</span>/8</span>
          {isHost && (
            <span className="inline-flex items-center gap-1 rounded-full bg-amber-400/15 px-2 py-0.5 text-[11px] font-semibold uppercase tracking-wide text-amber-300 ring-1 ring-inset ring-amber-300/30">
              You are Host
            </span>
          )}
        </div>
        ...
        <table className="w-full border-collapse text-left">
          ...
          <tbody className="divide-y divide-slate-700/70">
            ...
            {playerEntries.length === 0 && (
              <tr>
                <td colSpan={2} className="px-4 py-6 text-center text-sm text-slate-400">No players yet.</td>
              </tr>
            )}
          </tbody>
        </table>
